...
description: >-
  AI-assisted framework upgrade for a consumer repo. Pulls the latest .ai-framework
  submodule, runs the sync script, then intelligently reviews what changed — checking
  for repo-specific customizations in AGENTS.md that need to carry forward into
  updated skills, new skills that need AGENTS.md entries, and any breaking changes.
  Use after bumping the .ai-framework submodule or when prompted at session start.
---
