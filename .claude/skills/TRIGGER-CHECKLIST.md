...
| Before `create-plan` or `execute-task` when implementation pattern is absent from all KB layers | `research-implementation` | Suggest (don't auto-run); document before implementing |
...
| `research-implementation` | `/research-implementation [pattern]` | Research an unknown feature pattern before building; produces KB doc via `add-kb-doc` or short rule via `capture-convention` |
