...
**Status**: DONE — all 4 tasks + the direct dashboard fix complete, staging verification passed end-to-end on both iOS and Android (2026-09-14, real device, `ss1`, exact original repro). Squash-merged into `develop` (commit `2c24b1f9`), version bumped to `1.8.0+450`, `develop`/`qa` pushed to both `origin` and `bla` — QA build ready.
