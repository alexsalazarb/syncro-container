---
name: create-bug-plan
description: >-
  Creates a lightweight bug fix plan from a bug report or ticket.
  Runs an AI investigation phase, documents root cause, generates fix tasks with
  mandatory regression tests, and outputs the plan directory structure.
  Detects cross-project bugs and creates a master plan entry in the container
  when needed. Lighter-weight than create-plan — optimized for reactive bug fixes.
  Investigation artifacts become permanent institutional memory in git,
  discoverable by future agents and engineers via git log.
---
