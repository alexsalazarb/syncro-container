---
name: sync-master-plan
description: >-
  Syncs local plan progress to the hub's repos/{repo-key}.md and STATUS.md.
  Reads task statuses from the current repo's plan directories and updates
  the corresponding per-repo status file and aggregate status in the hub.
  Called automatically by execute-task on completion, or manually via
  /sync-master-plan.
---
