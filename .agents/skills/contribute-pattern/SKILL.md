...
description: >-
  Contributes a reusable pattern discovered in a consumer project back to the
  AI framework's knowledge-base, so all future projects of the same stack
  benefit. Has two modes: single (contribute one known pattern) and scan (read
  the entire project KB and identify everything promotable to the framework).
  Writes directly to .ai-framework/knowledge-base/{stack}/.
---
