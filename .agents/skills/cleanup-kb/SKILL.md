...
name: cleanup-kb
description: >-
  Removes KB documents that are no longer relevant to the project — library
  docs for libraries the project doesn't use, stale stubs seeded by init but
  never filled in, and orphaned entries in the KB README index. Safe: always
  asks for confirmation before deleting. Use after upgrading the framework or
  when the KB feels cluttered.
---
