...
name: execute-task
description: >-
  Executes a single task from a development plan. Loads task context from the
  plans directory, runs pre-flight checks, creates a git branch, follows
  implementation steps with adaptation protocol, runs verification, and updates
  status. Use when working on a specific plan task.
---
